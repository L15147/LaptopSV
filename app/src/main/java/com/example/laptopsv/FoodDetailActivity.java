            ...
            public void onClick(View view) {
                if(com.example.laptopsv.HomeActivity.listFoodOrder.size()>0){
                    for (int i = 0; i < com.example.laptopsv.HomeActivity.listFoodOrder.size(); i++){
                        ...
                        if(food.getID().equalsIgnoreCase(com.example.laptopsv.HomeActivity.listFoodOrder.get(i).getFood().getID())){
                            // -> cộng dồn số lượng :
                            com.example.laptopsv.HomeActivity.listFoodOrder.get(i).setSoLuong(com.example.laptopsv.HomeActivity.listFoodOrder.get(i).getSoLuong()+soLuong);
                ...
                com.example.laptopsv.HomeActivity.listFoodOrder.add(new FoodOrder(food,soLuong));
                Toast.makeText(FoodDetailActivity.this,"Thêm thành công",Toast.LENGTH_SHORT).show();
                onBackPressed();
            }
        });
